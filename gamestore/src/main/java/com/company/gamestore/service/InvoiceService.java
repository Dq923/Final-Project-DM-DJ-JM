...
public class InvoiceService {
    ...
    public InvoiceViewModel saveInvoice(InvoiceViewModel viewModel){
        ...
        /* Other method with if-else-if
        String type = viewModel.getItemType().toUpperCase();
         if(type.toUpperCase() == "GAME"){
            invoice.setItemType("GAME");
        }else if(type.toUpperCase() == "TSHIRT"){
            invoice.setItemType("TSHIRT");
        }else if(type.toUpperCase() == "CONSOLE"){
            invoice.setItemType("CONSOLE");
        }else{
        ...
        // Look up item id based on type
        if(invoice.getItemType() == "GAME"){
            Optional<Game> game = gameRepository.findById(invoice.getItemId());

            if(game.isEmpty()){ // if the game is not present throw exception
                throw new IllegalArgumentException();
            }

            if(game.get().getQuantity() < invoice.getQuantity()){ // if customer orders more than available throw exception
                throw new IllegalArgumentException();
            }

            game.get().setQuantity(game.get().getQuantity() - invoice.getQuantity());// update quantity of games
        }else
